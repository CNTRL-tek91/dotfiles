#!/usr/bin/env bash
# Re-applies the "cntrl" profile after waking from suspend.
#
...
#
# "cntrl" is saved manually from inside OpenRGB (Profiles > Save Profile),
...
# "cntrl" to survive, or the profile still holds the previous effect.
#
# This drives the ALREADY RUNNING instance through its tray menu rather than
# running `openrgb --profile cntrl`. That command starts a SECOND
...
exec "$HOME/.config/hypr/scripts/openrgb_tray.sh" "cntrl"
